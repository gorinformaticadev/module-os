...
export class ProdutosService {
  ...
  async create(data: any) {
    ...
    if (existing) {
      throw new Error(
        `O codigo "${data.code}" ja esta em uso por outro produto.`,
      );
    }
  ...
  private serializeProduct(product: any) {
    return {
      ...
      created_at:
        product.createdAt?.toISOString?.() ?? product.createdAt ?? null,
      updated_at:
        product.updatedAt?.toISOString?.() ?? product.updatedAt ?? null,
      deleted_at:
        product.deletedAt?.toISOString?.() ?? product.deletedAt ?? null,
    };
  }
}
