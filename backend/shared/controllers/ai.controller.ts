...
export class AiController {
  ...
  async analisarDescricao(
    @Req() req: ExpressRequest & { user: any },
    @Body() body: { descricao: string },
  ) {
    try {
      this.logger.log(
        `Solicitacao de analise de descricao para tenant ${req.user.tenantId}`,
      );
  ...
  async gerarLaudo(
    @Req() req: ExpressRequest & { user: any },
    @Body() body: { problema: string; notas: string },
  ) {
    try {
      this.logger.log(
        `Solicitacao de geracao de laudo para tenant ${req.user.tenantId}`,
      );
