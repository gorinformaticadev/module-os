...
export const ORDEM_SERVICO_UPLOAD_OPTIONS: Options = {
  ...
  fileFilter: (_req, file, callback) => {
    ...
    if (!rule) {
      callback(
        new BadRequestException("Tipo de arquivo nao permitido") as any,
        false,
      );
      return;
    }
...
export function persistTenantUpload(
  baseDir: string,
  tenantId: string,
...
export function persistTenantModuleUpload(
  ...
  const relativePath = safeSubdirectory
    ? `${safeSubdirectory}/${fileName}`
    : fileName;
...
export function resolveTenantUploadPath(
  ...
  const safeTenantId =
    tenantId === "." ? "." : sanitizeSegment(tenantId, "tenant");
...
export function resolveTenantModuleUploadPath(
  ...
  if (area === "produtos") {
    const legacyPath = resolveTenantUploadPath(
      resolveLegacyProductsRoot(),
      tenantId,
      relativeFilePath,
    );
...
export function removeTenantModuleUpload(
  ...
  const targetPath = resolveTenantModuleUploadPath(
    area,
    tenantId,
    relativeFilePath,
  );
...
export function assertTenantUploadAccess(
...
function normalizeUploadedBuffer(file: Express.Multer.File): Buffer {
  ...
  if (
    ...
    if (
      (bufferData as any).type === "Buffer" &&
      Array.isArray((bufferData as any).data)
    ) {
...
function sanitizeOriginalUploadName(filename: string) {
  ...
  if (
    !normalizedFilename ||
    INVALID_UPLOAD_NAME_REGEX.test(normalizedFilename)
  ) {
    throw new BadRequestException("Nome de arquivo invalido");
  }

  const extension = path
    .extname(path.basename(normalizedFilename))
    .toLowerCase();
...
function sanitizeRelativeDirectoryPath(rawPath?: string) {
  ...
  return segments
    .map((segment) => sanitizeSegment(segment, "diretorio"))
    .join("/");
}
...
function sanitizeRelativeFilePath(rawPath: string) {
  ...
  const safeDirectories = segments.map((segment) =>
    sanitizeSegment(segment, "diretorio"),
  );
...
function assertPathInsideRoot(rootPath: string, targetPath: string) {
  ...
  const startsWithRoot = normalizedTargetPath.startsWith(
...
function resolveLegacyModuleAreaRoot(area: OrdemServicoUploadArea) {
  ...
  return path.resolve(
    canonicalPaths.uploadsDir,
    ...ORDEM_SERVICO_UPLOAD_SEGMENT,
    safeArea,
  );
}
