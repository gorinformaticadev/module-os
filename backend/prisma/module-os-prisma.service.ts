...
function createModuleOsPrismaAdapter() {
  ...
  if (!connectionString) {
    throw new Error(
      "DATABASE_URL ausente para iniciar o Prisma local do module-os.",
    );
  }
  ...
  try {
    ...
    throw new Error(
      `Falha ao carregar @prisma/adapter-pg no module-os: ${reason}`,
    );
  }
}

@Injectable()
export class ModuleOsPrismaService
        ...
        $allModels: {
          async $allOperations({ model, operation, args, query }) {
            ...
            if (
              ...
              throw new Error(
                `[Security] RAW Query (${operation}) bloqueada no module-os.`,
              );
            }
            ...
            if (
              ...
              nextArgs.where = ModuleOsPrismaService.mergeTenantScope(
                nextArgs.where,
                tenantId,
              ) as any;
            }
            ...
            if (WRITE_OPERATIONS.has(operation)) {
              nextArgs.data = ModuleOsPrismaService.applyTenantData(
                nextArgs.data,
                tenantId,
              ) as any;
            }
  ...
  private static applyTenantData(data: any, tenantId: string) {
    if (Array.isArray(data)) {
      return data.map((entry) =>
        ModuleOsPrismaService.applyTenantData(entry, tenantId),
      );
    }
    ...
    if ("tenantId" in data && data.tenantId && data.tenantId !== tenantId) {
      throw new Error(
        "Tentativa de sobrescrever tenantId manualmente no module-os.",
      );
    }
    ...
    for (const nestedValue of Object.values(value)) {
      if (Array.isArray(nestedValue)) {
        nestedValue.forEach((item) =>
          ModuleOsPrismaService.assertTenantValue(item, tenantId),
        );
        continue;
      }
