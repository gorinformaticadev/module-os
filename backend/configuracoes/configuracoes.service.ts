...
export class ConfiguracoesService {
  ...
  async toggleTechnician(userId: string, isTechnician: boolean) {
    try {
      this.logger.log(
        `Alterando status de tecnico para usuario ${userId}: ${isTechnician}`,
      );
  ...
  async getProfilePermissions() {
    try {
      const permissions =
        await this.modulePrisma.mod_ordem_servico_profile_permissions.findMany({
      ...
      permissions.forEach((perm) => {
        const normalizedPermissionId = this.normalizePermissionId(
          perm.permissionId,
        );
        ...
        structuredPermissions[normalizedPermissionId][perm.profile] =
          perm.allowed === true;
      });
      ...
      for (const [permissionId, profiles] of Object.entries(
  ...
  async updateProfilePermissions(permissions: any) {
    try {
      ...
      for (const [permissionId, profiles] of Object.entries(permissions)) {
        ...
        for (const [profileName, allowed] of Object.entries(
  ...
  async createNotification(data: any) {
    try {
      const tenantId = this.getTenantIdOrThrow();
      const result =
        await this.modulePrisma.mod_ordem_servico_notification_schedules.create(
          {
            data: {
              tenantId,
              title: data.title,
  ...
  async getAiConfig() {
    try {
      ...
      if (maskedConfig.apiKey) {
        maskedConfig.apiKey =
          "********" + String(maskedConfig.apiKey).slice(-4);
      }
  ...
  async testAiConfig(testConfig: any) {
    try {
      ...
      const response = await this.aiService.callAI(
        {
          prompt:
            "Ola! Por favor, responda confirmando que voce esta funcionando corretamente.",
        },
        testConfig,
      );
  ...
  async getConfigurations() {
    try {
      const results =
        await this.modulePrisma.mod_ordem_servico_configs.findMany({
          orderBy: { key: "asc" },
          select: {
            key: true,
            value: true,
  ...
  private async upsertConfigValue(key: string, value: any) {
    ...
    const updateResult =
      await this.modulePrisma.mod_ordem_servico_configs.updateMany({
        where: { key },
        data: {
          value: configValue,
          updatedAt: new Date(),
  ...
  private async upsertIntegrationConfigValue(key: string, value: any) {
    ...
    const updateResult =
      await this.modulePrisma.mod_integracoes_configs.updateMany({
        where: { key },
        data: {
          value: configValue,
          updatedAt: new Date(),
