...
export class OrdensController {
  ...
  async findAll(
    ...
    try {
      this.logger.log(
        `🎯 [Controller] INÍCIO - Buscando ordens. Tenant: ${req.user?.tenantId}`,
      );

      const result = await this.ordensService.findAll(filters);

      this.logger.log(
        `🎯 [Controller] Service retornou ${result.data.length} ordens`,
      );
  ...
  async getDashboardData(
    ...
    try {
      this.logger.log(
        `Buscando dados do dashboard. Tenant: ${req.user?.tenantId}`,
      );
  ...
  async getTiposServico(
    @Req() req: AuthenticatedRequest,
  ): Promise<TipoServicoResponseDTO[]> {
    try {
      this.logger.log(
        `Buscando tipos de serviço. Tenant: ${req.user?.tenantId}`,
      );
  ...
  async getTiposEquipamento(
    @Req() req: AuthenticatedRequest,
  ): Promise<TipoEquipamentoResponseDTO[]> {
    try {
      this.logger.log(
        `Buscando tipos de equipamento. Tenant: ${req.user?.tenantId}`,
      );
  ...
  async getAlertasRetirada(
    ...
    try {
      this.logger.log(
        `Buscando alertas de retirada. Tenant: ${req.user?.tenantId}`,
      );
  ...
  async findOne(
    ...
    try {
      this.logger.log(
        `Buscando ordem de serviço ${id}. Tenant: ${req.user?.tenantId}`,
      );
  ...
  async getHistorico(
    ...
    try {
      this.logger.log(
        `Buscando histórico da ordem ${id}. Tenant: ${req.user?.tenantId}`,
      );
  ...
  async getWhatsAppEnvios(
    ...
    try {
      this.logger.log(
        `Buscando envios de WhatsApp da ordem ${id}. Tenant: ${req.user?.tenantId}`,
      );
      return await this.ordensService.getWhatsAppEnvios(id);
    } catch (error) {
      this.logger.error(
        `Erro ao buscar envios de WhatsApp da ordem ${id}:`,
        error,
      );
      throw error;
    }
  ...
  async registrarWhatsAppEnvio(
    ...
    try {
      this.logger.log(
        `Registrando envio de WhatsApp da ordem ${id}. Tenant: ${req.user?.tenantId}`,
      );
      return await this.ordensService.registrarWhatsAppEnvio(id, body);
    } catch (error) {
      this.logger.error(
        `Erro ao registrar envio de WhatsApp da ordem ${id}:`,
        error,
      );
      throw error;
    }
  ...
  async downloadPdf(
    ...
    try {
      this.logger.log(
        `Solicitação de PDF para ordem ${id}. Tenant: ${req.user?.tenantId}`,
      );
  ...
  async create(
    ...
    try {
      this.logger.log(
        `Criando nova ordem de serviço. Tenant: ${req.user?.tenantId}`,
      );

      // Validar se o cliente está ativo
      const clienteAtivo = await this.ordensService.isClienteAtivo(
  ...
  async update(
    ...
    try {
      this.logger.log(
        `Atualizando ordem de serviço ${id}. Tenant: ${req.user?.tenantId}`,
      );
      ...
      if (
        ...
        // FINALIZADA, CANCELADA, RETIRADO ou ABANDONADO
        throw new ForbiddenException(
          "Ordem de serviço finalizada, cancelada, retirada ou abandonada não pode ser editada",
        );
      }
  ...
  async updateStatus(
    ...
    try {
      this.logger.log(
        `Atualizando status da ordem ${id} para ${body.status}. Tenant: ${req.user?.tenantId}`,
      );
      ...
      const transicaoValida = await this.ordensService.validarTransicaoStatus(
        ordem.status,
        body.status,
      );
      if (!transicaoValida) {
        throw new BadRequestException(
          `Transição de status inválida: ${ordem.status} → ${body.status}`,
        );
      }
  ...
  async remove(
    ...
    try {
      this.logger.log(
        `Excluindo ordem de serviço ${id}. Tenant: ${req.user?.tenantId}`,
      );
  ...
  async aprovarOrcamento(
    ...
    try {
      this.logger.log(
        `Aprovando orçamento ${id}. Tenant: ${req.user?.tenantId}`,
      );
  ...
  async uploadFile(
    ...
    try {
      ...
      const persistedUpload = persistTenantModuleUpload(
        "ordens",
        safeTenantId,
        file,
        {
      ...
      return {
        url: buildTenantModuleUploadUrl(
          "ordens",
          safeTenantId,
  ...
  async serveFile(
    ...
    try {
      ...
      const normalizedFilePath = Array.isArray(filePath)
        ? filePath.join("/")
        : filePath;
      const safeFilePath = resolveTenantModuleUploadPath(
        "ordens",
        tenantId,
        normalizedFilePath,
      );
  ...
  async cleanupUploadedFiles(
    ...
    for (const url of urls) {
      const relativeFilePath = this.extractRelativeUploadPath(
        url,
        safeTenantId,
      );
      if (!relativeFilePath) {
    ...
    if (safeOrdemId) {
      ...
      if (!ordem) {
        throw new NotFoundException(
          "Ordem de serviço não encontrada para upload",
        );
      }
  ...
  private normalizeOrdemDirectoryName(ordemNumero: string): string {
    ...
    if (!normalized) {
      throw new BadRequestException(
        "Numero da ordem invalido para diretorio de upload",
      );
    }
  ...
  private extractRelativeUploadPath(
    url: string,
    tenantId: string,
  ): string | null {
    ...
    const relativeFilePath = normalizedPath
  ...
  async getStatusHistorico(
    ...
    try {
      this.logger.log(
        `Buscando histórico de status da ordem ${id}. Tenant: ${req.user?.tenantId}`,
      );
      return await this.ordensService.getStatusHistorico(id);
    } catch (error) {
      this.logger.error(
        `Erro ao buscar histórico de status da ordem ${id}:`,
        error,
      );
      throw error;
    }
  ...
  async getConservacao(
    ...
    try {
      this.logger.log(
        `Calculando conservação da ordem ${id}. Tenant: ${req.user?.tenantId}`,
      );
  ...
  async atualizarConservacao(
    ...
    try {
      this.logger.log(
        `Atualizando conservação da ordem ${id}. Tenant: ${req.user?.tenantId}`,
      );
  ...
  async getPagamentos(
    ...
    try {
      this.logger.log(
        `Buscando pagamentos da ordem ${id}. Tenant: ${req.user?.tenantId}`,
      );
  ...
  async registrarRetirada(
    ...
    try {
      this.logger.log(
        `Registrando retirada da ordem ${id}. Tenant: ${req.user?.tenantId}`,
      );
      ...
      if (ordem.status !== 6) {
        throw new BadRequestException(
          "Só é possível registrar retirada de ordens finalizadas",
        );
      }
  ...
  async getAlertasAbandono(
    ...
    try {
      this.logger.log(
        `Buscando alertas de abandono da ordem ${id}. Tenant: ${req.user?.tenantId}`,
      );
      return await this.ordensService.getAlertasAbandono(id);
    } catch (error) {
      this.logger.error(
        `Erro ao buscar alertas de abandono da ordem ${id}:`,
        error,
      );
      throw error;
    }
  ...
  async registrarAlertaAbandono(
    ...
    try {
      this.logger.log(
        `Registrando alerta de abandono para ordem ${id}. Tenant: ${req.user?.tenantId}`,
      );
      ...
      if (ordem.status !== 6) {
        throw new BadRequestException(
          "Só é possível registrar alertas para ordens finalizadas",
        );
      }
      ...
      this.logger.error(
        `Erro ao registrar alerta de abandono da ordem ${id}:`,
        error,
      );
      throw error;
    }
  ...
  async registrarAnexoAlerta(
    ...
    try {
      this.logger.log(
        `Registrando anexo para alerta ${alertaId}. Tenant: ${req.user?.tenantId}`,
      );
      return await this.ordensService.registrarAnexoAlerta(alertaId, body);
    } catch (error) {
      this.logger.error(
        `Erro ao registrar anexo do alerta ${alertaId}:`,
        error,
      );
      throw error;
    }
  ...
  async marcarComoAbandonado(
    ...
    try {
      this.logger.log(
        `Marcando ordem ${id} como abandonada. Tenant: ${req.user?.tenantId}`,
      );
      ...
      if (ordem.status !== 6) {
        throw new BadRequestException(
          "Só é possível marcar como abandonado ordens finalizadas",
        );
      }

      return await this.ordensService.marcarComoAbandonado(
        id,
        body.observacoes,
      );
    } catch (error) {
