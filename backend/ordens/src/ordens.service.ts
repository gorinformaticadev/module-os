...
export class OrdensService {
  ...
  private validateOrderFilters(filters: OrdemServicoFilters) {
    ...
    if (
      ...
      String(filters.search).trim().length < 2
    ) {
  ...
  private mapOrder(ordem: OrdemServico, responsavel: UserResponse | null) {
    const itens = this.parseJson<any[]>(ordem.itens as string | null, []);
    const equipamentoFotos = this.parseJson<string[]>(
      ordem.equipamentoFotos as string | null,
      [],
    );
    ...
    return {
      ...
      data_abertura:
        ordem.dataAbertura instanceof Date
          ? ordem.dataAbertura.toISOString()
          : null,
      data_previsao:
        ordem.dataPrevisao instanceof Date
          ? ordem.dataPrevisao.toISOString()
          : null,
      data_conclusao:
        ordem.dataConclusao instanceof Date
      ...
      equipamento_fotos: Array.isArray(equipamentoFotos)
        ? equipamentoFotos
        : [],
      ...
      created_at:
        ordem.createdAt instanceof Date ? ordem.createdAt.toISOString() : null,
      updated_at:
        ordem.updatedAt instanceof Date ? ordem.updatedAt.toISOString() : null,
      ...
      data_limite_retirada:
        ordem.dataLimiteRetirada instanceof Date
          ? ordem.dataLimiteRetirada.toISOString()
          : null,
      data_retirada:
        ordem.dataRetirada instanceof Date
          ? ordem.dataRetirada.toISOString()
          : null,
  ...
  async update(id: string, updateDto: UpdateOrdemServicoDTO) {
    ...
    if (updateDto.status !== undefined && updateDto.status !== atual.status) {
      const transicaoValida = await this.validarTransicaoStatus(
        atual.status,
        updateDto.status,
      );
      if (!transicaoValida) {
        throw new BadRequestException(
          `Transicao de status invalida: ${this.getStatusLabel(atual.status)} -> ${this.getStatusLabel(updateDto.status)}`,
        );
      }

      if (updateDto.status === StatusOS.FINALIZADA) {
        const valorFinal = Number(
          updateDto.valor_servico ?? atual.valor_servico ?? 0,
        );
        if (atual.status !== StatusOS.EM_EXECUCAO) {
          throw new BadRequestException(
            "So e possivel finalizar ordens em execucao",
          );
        }
        if (valorFinal <= 0) {
          throw new BadRequestException(
            "Valor do servico deve estar definido para finalizar",
          );
        }
      }
    }
    ...
    const data: any = {
      ...(updateDto.tipo_servico !== undefined
        ? { tipoServico: updateDto.tipo_servico }
        : {}),
      ...(updateDto.prioridade !== undefined
        ? { prioridade: updateDto.prioridade }
        : {}),
      ...(updateDto.descricao !== undefined
        ? { descricao: updateDto.descricao }
        : {}),
      ...(updateDto.observacoes_internas !== undefined
        ? { observacoesInternas: updateDto.observacoes_internas }
        : {}),
      ...(updateDto.observacoes_cliente !== undefined
        ? { observacoesCliente: updateDto.observacoes_cliente }
        : {}),
      ...(updateDto.valor_servico !== undefined
        ? { valorServico: updateDto.valor_servico }
        : {}),
      ...(updateDto.forma_pagamento !== undefined
        ? { formaPagamento: updateDto.forma_pagamento }
        : {}),
      ...(updateDto.data_previsao !== undefined
        ? { dataPrevisao: this.parseDate(updateDto.data_previsao) }
        : {}),
      ...(updateDto.usuario_responsavel_id !== undefined
        ? {
            usuarioResponsavelId: this.normalizeNullableResponsibleId(
              updateDto.usuario_responsavel_id,
            ),
          }
        : {}),
      ...(updateDto.equipamento_tipo !== undefined
        ? { equipamentoTipo: updateDto.equipamento_tipo }
        : {}),
      ...(updateDto.equipamento_marca !== undefined
        ? { equipamentoMarca: updateDto.equipamento_marca }
        : {}),
      ...(updateDto.equipamento_modelo !== undefined
        ? { equipamentoModelo: updateDto.equipamento_modelo }
        : {}),
      ...(updateDto.equipamento_serie !== undefined
        ? { equipamentoSerie: updateDto.equipamento_serie }
        : {}),
      ...(updateDto.equipamento_acessorios !== undefined
        ? { equipamentoAcessorios: updateDto.equipamento_acessorios }
        : {}),
      ...(updateDto.equipamento_estado !== undefined
        ? { equipamentoEstado: updateDto.equipamento_estado }
        : {}),
      ...(updateDto.formatacao_so !== undefined
        ? { formatacaoSo: updateDto.formatacao_so }
        : {}),
      ...(updateDto.formatacao_backup !== undefined
        ? { formatacaoBackup: updateDto.formatacao_backup }
        : {}),
      ...(updateDto.formatacao_backup_descricao !== undefined
        ? { formatacaoBackupDescricao: updateDto.formatacao_backup_descricao }
        : {}),
      ...(updateDto.formatacao_senha !== undefined
        ? { formatacaoSenha: updateDto.formatacao_senha }
        : {}),
      ...(updateDto.equipamento_fotos !== undefined
        ? { equipamentoFotos: this.stringifyJson(updateDto.equipamento_fotos) }
        : {}),
      ...(updateDto.itens !== undefined
        ? { itens: this.stringifyJson(updateDto.itens) }
        : {}),
      ...(updateDto.laudo_tecnico !== undefined
        ? { laudoTecnico: updateDto.laudo_tecnico }
        : {}),
      ...(updateDto.garantia_dias !== undefined
        ? { garantiaDias: updateDto.garantia_dias }
        : {}),
      ...(updateDto.origem_solicitacao !== undefined
        ? { origemSolicitacao: updateDto.origem_solicitacao }
        : {}),
      ...(updateDto.status !== undefined ? { status: updateDto.status } : {}),
      ...(updateDto.motivo_cancelamento !== undefined
        ? { motivoCancelamento: updateDto.motivo_cancelamento }
        : {}),
      ...(updateDto.status === StatusOS.FINALIZADA &&
      updateDto.status !== atual.status
  ...
  async updateStatus(
    ...
    if (!transicaoValida) {
      throw new BadRequestException(
        `Transicao de status invalida: ${this.getStatusLabel(statusAnterior)} -> ${this.getStatusLabel(novoStatus)}`,
      );
    }
    ...
    await this.ordemRepository.updateStatus(
      id,
      novoStatus,
      novoStatus === StatusOS.FINALIZADA ? new Date() : undefined,
      motivoCancelamento,
    );
  ...
  async getWhatsAppEnvios(ordemId: string) {
    ...
    const historico = await this.ordemRepository.getHistoricoByAcao(
      ordemId,
      this.WHATSAPP_HISTORY_ACTION,
    );
    const userIds = historico.map((item) => item.usuarioId).filter(Boolean);
    const usersById = await this.loadUsersMap(userIds);
  ...
  async registrarWhatsAppEnvio(
    ...
    return {
      ...
      mensagem,
      created_at:
        historico.createdAt instanceof Date
          ? historico.createdAt.toISOString()
          : null,
    };
  }
  ...
  async calcularConservacao(ordemId: string) {
    ...
    const prazoRetiradaDias = Number(
      (await this.getConfig("prazo_retirada_dias")) || 30,
    );
    ...
    const conservacaoHabilitada =
      (await this.getConfig("conservacao_habilitada")) === "true";
  ...
  async atualizarConservacao(
    ...
    return this.ordemRepository.updateConservacao(
      ordemId,
      valorConservacao,
      justificativa,
    );
  ...
  async validarRetirada(ordemId: string, pagamentos: PagamentoDTO[]) {
    ...
    if (ordem.status !== StatusOS.FINALIZADA) {
      throw new BadRequestException(
        "So e possivel registrar retirada de ordens finalizadas",
      );
    }
    ...
    if (pagamentos.length > 5) {
      throw new BadRequestException(
        "Maximo de 5 formas de pagamento permitidas",
      );
    }
    ...
    const valorConservacao = conservacao.emAtraso
      ? conservacao.valorConservacao
      : Number(ordem.valor_conservacao || 0);
  ...
  async registrarRetirada(ordemId: string, retiradaDTO: RetiradaDTO) {
    const actor = this.getActorOrThrow();
    const { ordem, conservacao, totalOS } = await this.validarRetirada(
      ordemId,
      retiradaDTO.pagamentos,
    );
    const valorConservacaoFinal =
      retiradaDTO.valor_conservacao !== undefined
  ...
  async getAlertasAbandono(ordemId: string) {
    ...
    return alertas.map((item) => ({
      ...
      data_envio:
        item.dataEnvio instanceof Date ? item.dataEnvio.toISOString() : null,
      ...
      anexos:
        item.anexos?.map((anexo) => ({
          ...
          created_at:
            anexo.createdAt instanceof Date
              ? anexo.createdAt.toISOString()
              : null,
  ...
  async registrarAlertaAbandono(ordemId: string, alertaDTO: AlertaAbandonoDTO) {
    ...
    if (ordem.status !== StatusOS.FINALIZADA) {
      throw new BadRequestException(
        "So e possivel registrar alertas para ordens finalizadas",
      );
    }
    ...
    if (alertaDTO.numero_alerta !== numeroEsperado) {
      throw new BadRequestException(
        `Alerta ${alertaDTO.numero_alerta} nao pode ser registrado. O proximo alerta esperado e o ${numeroEsperado}`,
      );
    }
    ...
    return {
      ...
      data_envio:
        alerta.dataEnvio instanceof Date
          ? alerta.dataEnvio.toISOString()
          : null,
  ...
  async validarAbandono(ordemId: string) {
    ...
    if (ordem.status !== StatusOS.FINALIZADA) {
      throw new BadRequestException(
        "So e possivel marcar como abandonado ordens finalizadas",
      );
    }
    ...
    if (alertas.length < 3) {
      throw new BadRequestException(
        `Sao necessarios 3 alertas registrados para marcar como abandonado. Alertas registrados: ${alertas.length}/3`,
      );
    }

    if (alertas.some((item) => !item.data_envio)) {
      throw new BadRequestException(
        "Todos os alertas devem ter data de envio registrada",
      );
    }
  ...
  async marcarComoAbandonado(ordemId: string, observacoes?: string) {
    ...
    await this.ordemRepository.registrarStatusHistorico({
      ...
      observacoes:
        observacoes ||
        "Equipamento marcado como abandonado apos 3 tentativas de contato",
    });
    await this.registrarHistorico(
      ordemId,
      "ABANDONO",
  ...
  private async registrarAlteracoesHistorico(
    ...
    const entries = Object.entries(updateDto).filter(
      ...
      const nextValue =
        Array.isArray(value) || typeof value === "object"
          ...
          : value == null
            ...
            : String(value);
      const prevValue =
        Array.isArray(previous) || typeof previous === "object"
  ...
  private getActorOrThrow() {
    ...
    if (!actor?.id || !actor.tenantId) {
      throw new BadRequestException(
        "Contexto de seguranca ausente para ordem de servico",
      );
    }
    return actor;
  }
