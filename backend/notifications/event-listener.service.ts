...
export class NotificationEventListenerService {
  ...
  @OnEvent("os.created")
  async handleOsCreated(payload: {
    tenantId: string;
    osId: string;
    data: any;
  }) {
    this.logger.log(`Evento os.created recebido para OS: ${payload.osId}`);
    await this.runForTenant(payload.tenantId, () =>
      this.processEventRules("CREATED", payload.osId, payload.data),
    );
  }

  @OnEvent("os.status_changed")
  async handleOsStatusChanged(payload: {
    tenantId: string;
    osId: string;
    oldStatus: string;
    newStatus: string;
    ...
    this.logger.log(
      `Evento os.status_changed recebido para OS: ${payload.osId} (${payload.oldStatus} -> ${payload.newStatus})`,
    );
    await this.runForTenant(payload.tenantId, () =>
      this.processEventRules("STATUS_CHANGED", payload.osId, payload.data),
    );
  }

  private async processEventRules(
    eventType: string,
    osId: string,
    osData: any,
  ) {
    try {
      const rules =
        await this.modulePrisma.mod_ordem_servico_notif_rules.findMany({
          where: {
            enabled: true,
            triggerType: "EVENT",
          },
      ...
      for (const rule of rules) {
        const config = this.normalizeJson<EventTriggerConfig>(
          rule.triggerConfig,
          {} as any,
        );
        if (
        ...
        this.logger.log(
          `Regra [${rule.title}] disparada para evento ${eventType}. Canal: ${rule.channel}`,
        );
  ...
  private async appendTechnicianTargets(
    ...
    const responsibleId =
      os?.usuario_responsavel_id || os?.usuarioResponsavelId;
    if (!responsibleId) {
      return;
    }
  ...
  private formatMessage(template: string, os: any): string {
    ...
    const map: Record<string, string | number> = {
      ...
      "{{data_previsao}}": os.data_previsao
        ? new Date(os.data_previsao).toLocaleDateString()
        : "N/A",
