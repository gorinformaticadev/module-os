...
export class NotificationSchedulerService implements OnModuleInit {
    ...
    try {
      await this.cronService.register(
        ...
        () => this.handleCron(),
        {
          ...
          description:
            "Busca OSs atrasadas ou que atingiram prazos configurados e realiza os disparos automaticos.",
  ...
  async handleCron() {
    ...
    try {
      ...
      for (const rule of activeRules) {
        await this.runForTenant(rule.tenantId, () =>
          this.processRule(rule as RuntimeRule),
        );
      }
    } catch (error: any) {
  ...
  private async processRule(rule: RuntimeRule) {
    if (
      rule.maxExecutions &&
      (rule.currentExecutions || 0) >= rule.maxExecutions
    ) {
      return;
    }
  ...
  private async processOffsetRule(rule: RuntimeRule) {
    const config = this.normalizeJson<RuleTriggerConfig>(
      rule.triggerConfig,
      {} as any,
    );

    if (
      config.reference === "BEFORE_DEADLINE" ||
      config.reference === "AFTER_DEADLINE"
    ) {
  ...
  private async processRelativeDeadline(rule: RuntimeRule, config: any) {
    ...
    ).mod_ordem_servico_ordens.findMany({
      where: {
        status: { notIn: [3, 4, 6] },
    ...
    for (const ordem of orders) {
      ...
      if (!alreadySent) {
        const success = await this.dispatchNotification(
          rule,
          ordem,
          fingerprint,
        );
  ...
  private async processOverdueOS(rule: RuntimeRule, config: any) {
    ...
    for (const ordem of overdueOrders) {
      ...
      const lastNotifiedAt = state?.updatedAt
      ...
      if (!state) {
        ...
        const ms =
          ((config.frequency.days || 0) * 86400 +
            (config.frequency.hours || 0) * 3600 +
            (config.frequency.minutes || 0) * 60 +
            (config.frequency.seconds || 0)) *
          1000;
        if (
          ms > 0 &&
          lastNotifiedAt &&
          Date.now() >= lastNotifiedAt.getTime() + ms
        ) {
          shouldNotify = true;
        }
      }
  ...
  private async dispatchNotification(
    ...
    if (this.isInSilenceWindow(config.silence_window)) {
      this.logger.log(
        `Regra [${rule.title}] silenciada pela Janela de Silencio`,
      );
      return false;
    }

    const recipientsList = await this.resolveRecipients(
      ordem,
      rule.recipients,
      rule.channel,
      rule.tenantId,
  ...
  private async resolveRecipients(
    ...
    for (const recipient of config) {
      switch (recipient.type) {
        case "CLIENT":
          await this.appendClientTargets(
            targets,
            ordem,
            tenantId,
            internalDelivery,
          ...
          break;
        case "TECHNICIAN":
          await this.appendTechnicianTargets(
            targets,
            ordem,
            tenantId,
            internalDelivery,
          ...
          break;
        case "ADMIN":
          await this.appendTenantRoleTargets(
            targets,
            tenantId,
            ["ADMIN"],
            internalDelivery,
        ...
        case "SUPER_ADMIN":
          ...
          break;
        case "CUSTOM":
          await this.appendCustomTargets(
            targets,
            recipient,
            tenantId,
            internalDelivery,
  ...
  private async appendClientTargets(
    targets: Set<string>,
    ordem: any,
    tenantId: string,
  ...
  private async appendTechnicianTargets(
    targets: Set<string>,
    ordem: any,
    tenantId: string,
    internalDelivery: boolean,
  ...
  private async appendCustomTargets(
    targets: Set<string>,
    recipient: any,
    tenantId: string,
    internalDelivery: boolean,
  ...
  private async resolveExplicitUserId(
    userId: string,
    tenantId: string,
    allowSuperAdmin = false,
  ): Promise<string | null> {
  ...
  private matchesOffsetRule(dataPrevisao: Date | null, config: any): boolean {
    ...
    const duration = config.offset_duration || {
      value: config.value,
      unit: config.unit,
    };
