...
export class NotificationStateService {
    ...
    async saveState(data: {
        ...
        const now = new Date();
        const updateResult = await this.modulePrisma.mod_ordem_servico_notif_states.updateMany({
            where: {
                ruleId: data.ruleId,
    ...
    async updateLastNotified(ruleId: string, ordemServicoId: string) {
        const count = await this.modulePrisma.mod_ordem_servico_notif_states.updateMany({
            where: {
                ruleId,
                ordemServicoId,
            },
        ...
        if (count.count === 0) {
            this.logger.warn(`Estado de notificacao nao encontrado para rule=${ruleId}, ordem=${ordemServicoId}`);
        }
    }
}
