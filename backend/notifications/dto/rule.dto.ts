...
export function validateCreatePayload(data: any): CreateNotificationRuleDto {
  ...
  if (forbiddenUsed.length > 0) {
    throw new BadRequestException(
      `Campos nao permitidos: ${forbiddenUsed.join(", ")}`,
    );
  }
  ...
  if (triggerType && !ALLOWED_TRIGGER_TYPES.includes(triggerType)) {
    throw new BadRequestException(
      `trigger_type deve ser um dos: ${ALLOWED_TRIGGER_TYPES.join(", ")}`,
    );
  }
  ...
  if (channel && !ALLOWED_CHANNELS.includes(channel)) {
    throw new BadRequestException(
      `channel deve ser um dos: ${ALLOWED_CHANNELS.join(", ")}`,
    );
  }

  const normalizedTriggerConfig = normalizeTriggerConfig(
    triggerType,
    data.trigger_config || data.triggerConfig,
  );
  ...
  if (data.max_executions !== undefined) {
    if (data.max_executions === null || data.max_executions === "") {
      ...
      if (isNaN(parsed) || parsed < 0) {
        throw new BadRequestException(
          "max_executions deve ser um numero nao negativo",
        );
      }
  ...
  return {
    ...
    message_template:
      data.message_template?.trim() || data.messageTemplate?.trim() || null,
...
export function validateUpdatePayload(
  data: any,
): Partial<CreateNotificationRuleDto> {
  if (!data || typeof data !== "object") {
    throw new BadRequestException("Payload deve ser um objeto valido");
  }
  ...
  if (data.recipients !== undefined) {
    if (data.recipients === null) {
      ...
      result.recipients = data.recipients.map((r: any, index: number) => {
        if (!r.type || typeof r.type !== "string") {
          throw new BadRequestException(
            `recipient[${index}].type e obrigatorio`,
          );
        }
        return {
          type: r.type,
...
function normalizeTriggerConfig(
  ...
  return {
    ...getDefaultTriggerConfig(triggerType || "EVENT"),
    ...triggerConfig,
  };
}
