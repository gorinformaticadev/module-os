...
export class NotificationRuleService {
  ...
  async findOne(id: string) {
    const rule =
      await this.modulePrisma.mod_ordem_servico_notif_rules.findFirst({
        where: { id },
      });
  ...
  async create(data: CreateNotificationRuleDto) {
    this.logger.log(
      `Creating notification rule. Data: ${JSON.stringify(data)}`,
    );
  ...
  async update(id: string, data: Partial<CreateNotificationRuleDto>) {
    ...
    if ("message_template" in data)
      payload.messageTemplate = data.message_template;
    if ("max_executions" in data)
      payload.maxExecutions = data.max_executions ?? null;
    if ("expires_at" in data || "expiresAt" in (data as any)) {
      payload.expiresAt = this.normalizeDate(
        (data as any).expires_at ?? (data as any).expiresAt,
      );
    }
    if ("trigger_config" in data || "triggerConfig" in (data as any)) {
      payload.triggerConfig = ((data as any).trigger_config ??
        (data as any).triggerConfig ??
        {}) as unknown as Prisma.InputJsonValue;
    }
