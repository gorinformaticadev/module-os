...
INSERT INTO mod_ordem_servico_configs (tenant_id, config_key, config_value)
SELECT 
    t.id as tenant_id,
    'condicoes_execucao' as config_key,
    'O serviço será executado conforme descrito acima. Eventuais alterações serão comunicadas ao cliente. A garantia cobre apenas defeitos relacionados ao serviço executado.' as config_value
FROM tenants t
WHERE NOT EXISTS (
    SELECT 1 FROM mod_ordem_servico_configs 
    WHERE tenant_id = t.id AND config_key = 'condicoes_execucao'
);
